    ...
    <div className="min-h-full bg-app pb-24">
      ...
      <div className="px-4 space-y-3">
        {isLoading && (
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="bg-white rounded-2xl p-4 h-20 animate-pulse shadow-[var(--shadow-card)]" />
            ))}
          </div>
        )}
        ...
        {!isLoading && grupos.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="font-bold text-ink mb-1">Sin grupos</p>
            <p className="text-[#6B6B6F] text-sm mb-5">Crea un grupo para dividir gastos</p>
        ...
        {grupos.map((g) => (
          <button
            ...
            className="w-full bg-white rounded-2xl p-4 flex items-center gap-3 shadow-[var(--shadow-card)] text-left active:scale-[0.98] transition-transform"
          >
            <div className="w-12 h-12 rounded-2xl bg-[#F2F2F4] flex items-center justify-center text-2xl flex-shrink-0">
              {g.emoji}
            </div>
            ...
            {g.cerrado_en && (
              <span className="text-xs text-[#6B6B6F] bg-[#F2F2F4] px-2 py-1 rounded-full">Cerrado</span>
            )}
          </button>
        ))}
      </div>
      ...
      {showNuevo && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl p-5 space-y-4 max-h-[85vh] overflow-y-auto">
            <h2 className="font-bold text-lg text-ink">Nuevo grupo</h2>
            ...
            {/* Moneda */}
            <div>
              <p className="text-xs text-[#6B6B6F] mb-2 font-medium">Moneda principal</p>
              <select
                value={moneda}
            ...
            <button
              ...
              className={clsx(
                ...
                esCuentaReal ? "bg-ink text-white" : "bg-[#F2F2F4] text-ink"
              )}
            >
              <span className="text-xl">💳</span>
              <div className="flex-1 text-left">
                <p className="font-semibold text-sm">Cuenta real</p>
                <p className={clsx("text-xs", esCuentaReal ? "text-white/70" : "text-[#6B6B6F]")}>
              ...
              <div className={clsx("w-5 h-5 rounded-full border-2 flex items-center justify-center", esCuentaReal ? "border-white" : "border-[#A0A0A4]")}>
                {esCuentaReal && <div className="w-2.5 h-2.5 rounded-full bg-white" />}
              </div>
            </button>
            ...
            {amigos.length > 0 && (
              <div>
                <p className="text-xs text-[#6B6B6F] mb-2 font-medium">Añadir amigos</p>
                  ...
                  {amigos.map((a) => (
                    <button
                      ...
                      className={clsx(
                        ...
                        miembrosIds.includes(a.usuario_id) ? "bg-ink text-white" : "bg-[#F2F2F4] text-ink"
                      )}
                    >
                      <span className="text-lg">{a.avatar_emoji}</span>
                      <span className="font-semibold text-sm">{a.nombre}</span>
                      <span className={clsx("text-xs ml-auto", miembrosIds.includes(a.usuario_id) ? "text-white/70" : "text-[#6B6B6F]")}>
                        @{a.usuario_unico}
                      </span>
                    </button>
                  ))}
