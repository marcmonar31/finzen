...
function SaludScore({ score, nivel, factores }: { score: number; nivel: string; factores: FactorSalud[] }) {
  ...
  return (
    <div className={clsx("rounded-2xl p-4 shadow-sm", BG[nivel] ?? "bg-gray-50")}>
      <h3 className="text-xs font-semibold text-ink/40 uppercase tracking-wide mb-3">Salud financiera</h3>
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-32 h-16 flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end pb-1">
            ...
            <span className="text-xs text-ink/40 capitalize">{nivel}</span>
          </div>
        </div>
        {/* Factores */}
          ...
          {factores.map((f) => (
            <div key={f.nombre}>
              <div className="flex justify-between text-xs mb-0.5">
                <span className="text-ink/60 truncate">{f.nombre}</span>
                <span className="font-semibold text-ink flex-shrink-0 ml-1">{f.puntos}/{f.max_puntos}</span>
              </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-ink">Predicción de saldo</h3>
      </div>
        ...
        {datos.map((d) => {
          ...
          return (
            <div key={d.dias} className="text-center bg-gray-50 rounded-xl p-2">
              <p className="text-xs text-ink/40 mb-1">+{d.dias} días</p>
              <p className={clsx("text-sm font-bold", saldo >= 0 ? "text-ink" : "text-red-500")}>
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-ink">Suscripciones detectadas</h3>
      ...
      <div className="space-y-2">
        {subs.map((s) => (
          <div key={s.concepto} className="flex items-center justify-between">
            <p className="text-sm text-ink truncate">{s.concepto}</p>
            <div className="text-right flex-shrink-0 ml-2">
              <p className="text-sm font-semibold text-ink">{formatCurrency(s.importe_medio, s.moneda)}/mes</p>
              <p className="text-xs text-ink/40">{s.num_ocurrencias} pagos</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-ink">Gastos hormiga</h3>
      ...
      <div className="space-y-2">
        {items.slice(0, 5).map((item) => (
          <div key={item.concepto} className="flex items-center justify-between">
            <div>
              <p className="text-sm text-ink">{item.concepto}</p>
              <p className="text-xs text-ink/40">{item.num_ocurrencias_mes}× este mes</p>
            </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-red-100">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-ink">Gastos inusuales</h3>
      ...
      <div className="space-y-2">
        {anomalias.map((a) => (
          <div key={a.movimiento_id ?? a.concepto} className="flex items-center justify-between">
            <div>
              <p className="text-sm text-ink">{a.concepto}</p>
              <p className="text-xs text-ink/40">{new Date(a.fecha + "T00:00:00").toLocaleDateString("es-ES")} · {parseFloat(a.factor).toFixed(1)}× sobre la media</p>
            </div>
  ...
  return (
    <div className="min-h-full bg-app pb-24">
      ...
      <div className="px-4 max-w-2xl mx-auto space-y-3">
        {isLoading ? (
          <div className="text-center py-12 text-ink/40 text-sm">Analizando tus finanzas...</div>
        ) : !data ? (
          <div className="text-center py-12">
            <Zap size={36} className="mx-auto text-ink/20 mb-3" />
            <p className="text-ink/50 text-sm">Añade movimientos para ver insights</p>
          </div>
        ) : (
          <>
