...
function TablaAmortizacion({ deudaId }: { deudaId: string }) {
  ...
  if (isLoading) return <div className="text-xs text-fg/40 py-2 text-center">Cargando cuotas...</div>;
...
function DeudaCard({ deuda }: { deuda: DeudaOut }) {
  ...
  async function handleArchivar() {
    try { await archivar.mutateAsync(deuda.id); showFlash("Deuda eliminada", "delete"); }
    catch { showFlash("Error al eliminar", "error"); }
  }
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-fg/40">{TIPO_LABEL[deuda.tipo] ?? deuda.tipo}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="mt-3 flex items-end justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            {deuda.num_cuotas && (
              <span className="text-xs text-fg/50">{deuda.num_cuotas} cuotas</span>
            )}
            {tasa > 0 && (
              <span className="text-xs text-orange-500">{tasa.toFixed(2)}% TAE</span>
            )}
            <span className="text-xs text-fg/40">Día {deuda.dia_cuota} de cada mes</span>
          </div>
        </div>
        {deuda.num_cuotas && (
          <button
...
function NuevaDeudaSheet({ onClose }: { onClose: () => void }) {
  ...
  async function handleCrear() {
    ...
    try {
      ...
      showFlash("Deuda registrada");
      onClose();
    } catch { showFlash("Error al crear la deuda", "error"); }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-surface rounded-t-3xl p-6 max-h-[85vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-bold text-fg mb-4">Registrar deuda</h2>
        <div className="space-y-3">
          <input
            className="w-full px-3 py-2 rounded-xl border border-gray-200 text-fg focus:outline-none"
            placeholder="Nombre (ej. Hipoteca banco)" value={nombre} onChange={(e) => setNombre(e.target.value)}
          />
          <select
            className="w-full px-3 py-2 rounded-xl border border-gray-200 text-fg bg-surface focus:outline-none"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-fg/40">Tipo interés anual (%)</label>
              <input
                type="number" inputMode="decimal" placeholder="0"
                className="w-full mt-1 px-3 py-2 rounded-xl border border-gray-200 text-fg focus:outline-none"
                value={tasa} onChange={(e) => setTasa(e.target.value)}
              />
            </div>
            <div>
              <label className="text-xs text-fg/40">Número de cuotas</label>
              <input
            ...
            <div>
              <label className="text-xs text-fg/40">Fecha inicio</label>
              <input type="date"
                className="w-full mt-1 px-3 py-2 rounded-xl border border-gray-200 text-fg focus:outline-none"
            ...
            <div>
              <label className="text-xs text-fg/40">Día de cuota (mes)</label>
              <input
        ...
        <div className="flex gap-3 mt-5">
          <button className="flex-1 py-3 rounded-2xl bg-gray-100 text-fg font-semibold" onClick={onClose}>Cancelar</button>
          <button
      ...
      <div className="px-4 max-w-2xl mx-auto">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-fg/50 text-sm mt-3">Sin deudas registradas</p>
            <p className="text-fg/30 text-xs mt-1">Si tienes préstamos o hipotecas, añádelos para ver el cuadro de amortización</p>
          </div>
        ) : (
          <div className="space-y-3">
